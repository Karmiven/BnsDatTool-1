	...
	public class SeqInfo<SeqCell> : List<SeqCell>, ICloneable where SeqCell : Seq.SeqCell
	{
		...
		public new void Add(SeqCell item)
		{
			if (item.Key >= 0 && !string.IsNullOrWhiteSpace(item.Alias))
			{
				...
				else LogWriter.WriteLine($"== CRASH == 枚举Initialize错误，别名重复 ({this.Name} => { item.Alias })");
			}

			if (!this.ht_id.ContainsKey(item.Key)) this.ht_id.Add(item.Key, item);
			else LogWriter.WriteLine($"== CRASH == 枚举Initialize错误，主键重复 [{this.Name} => {item.Alias} ({item.Key})]");
